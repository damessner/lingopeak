...
export default function TeacherDashboardClient({
  ...
  // Worksheets custom builder state
  const [worksheets, setWorksheets] = useState<any[]>([]);
  const [worksheetsLoading, setWorksheetsLoading] = useState(false);
  const [isEditingWorksheet, setIsEditingWorksheet] = useState(false);
            ...
            {isEditingWorksheet ? (
              <WorksheetBuilder
                ...
                onSave={() => {
                  setIsEditingWorksheet(false);
                  setCurrentWorksheet(null);
                  displayMessage('Worksheet saved and published successfully!', 'success');
                  fetchWorksheets();
                }}
                onCancel={() => {
                  setIsEditingWorksheet(false);
              ...
              <section className="bg-slate-900/40 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-6 print:bg-transparent print:border-none print:shadow-none">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-800/80 pb-4 no-print">
                  ...
                  <button
                    ...
                    className="bg-indigo-600 hover:bg-indigo-500 text-white font-bold border border-indigo-400/20 text-xs py-2.5 px-5 rounded-xl cursor-pointer transition-all shadow-md flex items-center gap-2"
                  >
                            ...
                            <tr key={ws.id} className="hover:bg-slate-900/10">
                              ...
                              <td className="py-3 px-4 text-right space-x-2 no-print">
                                <button
                                  ...
                                  className="bg-slate-900 hover:bg-slate-800 text-slate-300 border border-slate-800 hover:border-slate-700 py-1.5 px-3 rounded-lg text-[10px] font-bold cursor-pointer transition-colors"
                                >
                                  Edit ✏️
                                </button>
                                <button
